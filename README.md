...
| --- | --- | --- |
| `article1_dualwater_dielectric_nmr` | Dual-water joint inversion: excess-clay & effective-water conductivity terms, Swb↔Qv↔CEC, NMR clay-bound water (T2 < 3 ms), Co (Sw=1) & the Co−Cw Qv, m0 inversion, the end-to-end six-step workflow, the Fig. 7 Sxo-dominant sensitivity, the Appendix n-vs-m trade-off, Sw solve | Tan, Lafferty & Neville, pp. 14–23 |
| `article2_pc_resistivity_index_carbonate` | Resistivity index RI = Sw⁻ⁿ and saturation-exponent fit (per displacement cycle), Archie formation factor & cementation-exponent fit, Sw = RI^(−1/n), movable-oil endpoints, bar↔psi reconciliation, Leverett J-function (with the carbonate caveat) & Washburn pore-throat radius | Dernaika et al., pp. 24–30 |
| `article3_nanofluid_imbibition` | Young's law contact angle & cosine, wettability class, Young-Dupré work of adhesion, Young-Laplace capillary pressure (barrier↔driving force), capillary-vs-gravity shape test, first-order recovery, CMC effectiveness & nanofluid incremental recovery (~10% IOIP) | Roustaei, pp. 31–37 |
...
| `article5_thermal_conductivity_velocity` | Voigt/Reuss bounds & Hill average, petrographic-code aspect ratios, crack density by count (N/V·r³) & by porosity/aspect-ratio, Budiansky-O'Connell cracked moduli, Sen depolarization, Clausius-Mossotti λ, the λ-from-Vp two-step estimator, the Table-4 Vp→λ regression & <15% accuracy | Gegenhuber & Schön, pp. 51–56 |
